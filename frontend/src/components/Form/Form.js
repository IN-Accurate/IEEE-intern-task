...
const Form = () => {
  ...
  const sendForm = (event) => {
    ...
    const checkedServices = [
      ...servicesField.current.querySelectorAll("input:checked"),
    ...
    if (!phoneNumberPattern.test(phoneNumber)) {
      ...
      validationSchema
        ...
        .then(() => {
          ...
          axios
            .post(
              "https://mint-forms.ieee-mint.org/api/form/addresponse?formId=task",
              formData,
              {
                formId: "task",
                headers: {
    ...
    <form className="form" onSubmit={sendForm}>
        ...
        <>
          <div className="form-header">
            <MuiThemeProvider theme={THEME}>
              <Typography>Let's level up your brand,together</Typography>
            </MuiThemeProvider>
          </div>
          ...
          <button type="submit" style={{ width: "80%" }} onClick={sendForm}>
            Send message
          </button>
        </>
      )}
      {isMobile && (
        <>
          ...
          <label htmlFor="services" style={{ marginTop: "10px" }}>
            Services
          </label>
          <div className="checkbox-container" ref={servicesField}>
            <div style={{ display: "flex", flexDirection: "row", gap: 2 }}>
